...
def train(model, optimizer, criterion, stage=1):
    ...
    for sample_batched in loop:
        ...
        if stage == 0: # stage 0 is to train the Forward MIM only
            gmos = None
        ...
        if stage == 1:  # stage 1 is to train the full network, include the backward MIM
            y_loss = criterion(preds.flatten().float(), gmos.float())

            if args.contrast:
                loss = y_loss + args.alpha * nce - args.beta * lld
            else:
                loss = y_loss
            if i_batch > mem_size:
                loss -= args.beta * H
...
if __name__ == '__main__':
    dataset   = 'AGIQA-3k'      # ['AGIQA-3k', 'AIGCIQA2023', 'PKU-AIGIQA']
    ...
    ROUND     = 10              # 10 fold train-test split cross validation
    num_epoch = 20              # number of epochs for every train-test split valiadation
    step_size = 4               # Number of steps of lr decay
    ...
    checkpoint_dir = './weights_final/MI-AIGIQA_%s_alpha%s_beta%s_batch%s_mainlr%s_%sfold_%s' \
                     % (dataset, alpha, beta, train_batch, main_lr, ROUND, mos_type)  # Saving the weight and the train log
    ...
    for session in range(0, ROUND):
        ...
        aigc_train_csv = os.path.join('./Database/%s' % dataset, str(session+1), 'train.csv')
        aigc_val_csv = os.path.join('./Database/%s' % dataset, str(session+1), 'val.csv')

        if   dataset == 'AGIQA-1k':
            aigc_set = '../AIGIQA/IQA_models/IPCE/data/%s/Image' % dataset  # We also support AGIQA-1k

        elif dataset == 'AGIQA-3k':
            aigc_set = '../AIGIQA/IQA_models/IPCE/data/%s/Image' % dataset
        ...
        # Set the train & validation set
        aigc_train_loader = set_dataset_pyiqa(dataset, aigc_train_csv, train_batch, aigc_set, num_workers, preprocess2, mos_type, False)
        aigc_val_loader   = set_dataset_pyiqa(dataset, aigc_val_csv, val_batch, aigc_set, num_workers, preprocess2, mos_type, True)

        train_loaders = [aigc_train_loader]

        result_pkl = {}
        for epoch in range(0, num_epoch):
            # maximize likelihood in stage0
            if args.contrast:
                avg_loss_stage0 = train(model, optimizer_mmilb, criterion, 0)
            scheduler_mmilb.step()

            # minimize all losses in stage1
            ...
            log_and_print(base_logger,'best quality result:{}, plcc{}, srcc:{}\n'.format(best_result['quality'], best_result['plcc'], best_result['srcc'],))

        best_result_list.append(best_result)

        # Clean up memory to prevent state leaks and OutOfMemoryErrors (OOM).
        del model, optimizer_main, optimizer_mmilb, scheduler_main, scheduler_mmilb
        del aigc_train_loader, aigc_val_loader
    ...
    avg_srcc = avg_srcc / ROUND  # Compute the avg SRCC among all splits
    avg_plcc = avg_plcc / ROUND
    log_and_print(base_logger,'all_finished,average plcc:{}, srcc:{},'.format(avg_plcc, avg_srcc))